...
# correlation
df_corr <- cor(trained, method = "pearson")
...
broom::tidy(coef(step_fit$finalModel, 22)) -> step_coef

step_coef <- step_coef[-1, ]
lm_formula <- as.formula(paste("train_logy ~ ",paste(step_coef$names, collapse="+"), sep = ""))
...
step_fit #0.87, 0.1427, 0.100

step_pred <- predict(step_lm, tested)
...
summary(earth_fit) 
plot(earth_fit)
earth::evimp(earth_fit) # 3 outliers 
